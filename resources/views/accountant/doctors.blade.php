@extends('new.layout')
@section('title')
صندوق امروز به تفکیک پزشک پیشفرض
@stop
...
@section('content')
	...
	<div class="m-content" style="width: 100%; background-color: white;">
		<div class="col-md-12">
			...
			<br><br>
			<h4>جزئیات صندوق امروز به تفکیک پزشک پیشفرض</h4><br>
						...
						@foreach($services as $row)
							@php
								...
								foreach($query as $key) {
									if($key->service_id == $row->id && $u->id == $key->doctor_id) {
										$tariff += \DB::table('insurance_service')->where('service_id', $row->id)->first()->tariff;
										$count+=$key->count;
										if($key->discount >0)
							...
							@endphp
							@if($count > 0)
							<tr align="center" @if(fmod($i, 2) == 1) style="background-color: #ccc;" @endif>
								<td>
									{{ \App\Helpers\ConvertNumber::convertToPersian($i++) }}
								</td>
								<td>
									{{ \App\Models\Role::find($u->role_id)->title }} {{ \App\Models\User::find($u->id)->name }}
								</td>
								<td>
									{{ $row->name }}
								</td>
