...
@section('title')
	@if(isset($model))
	ویرایش نوبت خدمات کلینیکی
	@else
	ایجاد نوبت خدمات کلینیکی
	@endif
@stop
@section('content')
						...
						<div class="row">
							...
							<div class="col-md-6">
								...
								<div class="m-portlet m-portlet--tab">
									...
									@else	
										{!! Form::open(['action' => ['TurnController@storeClinic'], 'class' => 'm-form m-form--fit m-form--label-align-right','files' => true,'id' => 'myForm','onkeydown' => 'return event.key != "Enter";']) !!}
									@endif		
										...
										@endif	
											...
											<div class="form-group m-form__group" id="servs">
												<label for="">انتخاب خدمت</label><br>
												...
												<tr align="center">
													<td>
													<input style="margin-right: 1%;" id="service_{{ $service->id }}" type="checkbox" class="services" name="services[]" value="{{ $service->id }}"> {{ $service->name }}
												</td>
												<td>
													<input type="number" class="" name="count_{{ $service->id }}" placeholder="تعداد">
												</td>
											</tr>
												@endforeach
											</tbody>
													...
													<input type="text" name="discount" style="margin-right: 25%; display: none;" class="m-input" placeholder="تخفیف">
												<hr>
												<h3 id="count"></h3>
											</div>
...
<script type="text/javascript">
  $(function() {
	...
	<script>
		$(document).on("change", "#patients", function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                	if(data == 'oops!') {
                		window.location.replace('{{ url('patients') }}'+'/'+$('#patients').val()+'/edit?redirect=turns');
                	} else {
                    	$("#causes").html(data);
                    	$("#patient_name").val('');
                    }
    ...
    var checked = $("#servs input:checked").length > 0;
    if (!checked){
